...
saveButton.addEventListener("click", function() {
    ...
    if (confirmSave) {
        ...
        let parsedData = JSON.parse(localStorage.getItem("savedData")) || {}; // Initialize as empty object
        parsedData.sheets = parsedData.sheets || []; // Initialize sheets array if not present

        // Find the index of the current sheet being edited
        const currentIndex = parsedData.sheets.findIndex(sheet => sheet.title === serializedData.title);

        if (currentIndex !== -1) {
            // Replace the existing entry with the new data
            parsedData.sheets[currentIndex] = serializedData;

            // Update the savedData directly
            localStorage.setItem("savedData", JSON.stringify(parsedData));
        }

        // Update the currentSheetForEdit if the edited sheet is the same as the current one
        const currentSheetForEdit = localStorage.getItem("currentSheetForEdit");
        if (currentSheetForEdit && JSON.parse(currentSheetForEdit).title === serializedData.title) {
            localStorage.setItem("currentSheetForEdit", JSON.stringify(serializedData));
        }
